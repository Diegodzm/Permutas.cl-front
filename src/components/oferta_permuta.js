    ...
    return (
        <Container>
            ...
            <div className="section mt-4" style={{ backgroundColor: '#f0fcfb', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
                {(productosOferta.length > 0 || productoSeleccionado) && <h2 className="text-center mb-4" style={{ color: '#006400' }}>Productos para permutar</h2>}
                <Row className="justify-content-between align-items-center">
                            ...
                            <Card key={index} style={{ width: '100%', backgroundColor: '#dfffd8', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', borderRadius: '10px', marginRight: index === productosOferta.length - 1 ? 0 : '20px' }}>
                                <Card.Body>
                                    ...
                                    <div className="card-img-container" style={{ maxHeight: '200px', overflow: 'hidden' }}>
                                        <Card.Img className="cardimg mx-auto" variant="top" src={product.photo} style={{ objectFit: 'cover', width: '100%', height: '100%' }} />
                                    </div>
                                    <Card.Text style={{ color: '#4f4f4f' }}>Descripción: {product.product_info}</Card.Text>
                                    <Card.Text style={{ color: '#4f4f4f' }}>Precio: {numberWithDots(product.price)}</Card.Text>
                    ...
                    <Col xs={6} className="d-flex justify-content-start">
                        {productoSeleccionado && (
                            <Card style={{ width: '100%', backgroundColor: '#e6f3f3', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', borderRadius: '10px' }}>
                                <Card.Img className="cardimg mx-auto" variant="top" src={productoSeleccionado.photo} />
                                <Card.Body>
                                    ...
                                    <Card.Text style={{ color: '#4f4f4f' }}>Precio: {numberWithDots(productoSeleccionado.price)}</Card.Text>
                ...
                {productosOferta.length > 0 && productoSeleccionado && (
                    <div className="text-center mt-4">
                        ...
                        }} style={{ backgroundColor: '#20c997', borderColor: '#20c997', color: '#fff', marginRight: '10px',marginTop:'10px' }}>Ofrecer Intercambio</Button>

                        <Button className="mt-2"variant="warning" style={{ color: '#fff', backgroundColor: '#ffc107', borderColor: '#ffc107' }} onClick={handleUndoButtonClick}>Deshacer Operación</Button>
                    </div>
                )}
            </div>
                    ...
                    {productoSeleccionado ? null : (
                        store.publishedProducts && store.publishedProducts[store.productIndex] && (
                            <Col xs={12} className='mb-4'>
                                <Card style={{ width: '100%', maxWidth: '600px', margin: '0 auto', backgroundColor: '#ffffff', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', borderRadius: '10px' }}>
                                    <Card.Img className="cardimg mx-auto" variant="top" src={store.publishedProducts[store.productIndex].photo} />
                                    <Card.Body>
                                        ...
                                        <Card.Text style={{ color: '#666' }}>
                                            Precio: {numberWithDots(store.publishedProducts[store.productIndex].price)}
                                        </Card.Text>
